...
For corporate source-row security changes, preserve the compact `SEC-*` summary first, then document slim source-row coverage below it by reference: catalog snapshot/path, `expectedSourceIdCount: 155`, section/group references, valid compact mappings, lifecycle status, evidence owners, downstream traceability, safe-evidence policy, and evidence-backed `N/A` rationale. Do not duplicate the general 96-control review matrix, materialize the exhaustive 155-row Source ID inventory, or require legacy security validators; full Source ID expansion belongs to `sdd-review-security`.
