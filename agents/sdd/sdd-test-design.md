...
For corporate source-row security changes, plan static/manual evidence when tooling is unavailable; consume slim design coverage by catalog snapshot/path, `expectedSourceIdCount: 155`, source-row groups or catalog-backed references, compact `SEC-*` mappings, lifecycle status, mandatory/advisory severity, expected evidence owners, warning preservation, and evidence-backed `N/A` checks. Do not require design or test-design to duplicate the full 155-row matrix; exhaustive expansion belongs to `review-security-report.md`. Mandatory source-row evidence gaps block; missing runtime/build/lint/type/format/coverage tooling must be reported as unavailable evidence, not passing evidence.
