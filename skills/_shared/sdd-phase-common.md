...
| `sdd-review-security` | design with `## Secure Development Design` narrative rules + non-blocking review-report canonical JSON/derived Markdown identity + apply-progress/evidence + changed-file context | tasks, test-design, catalog | `review-security-report` canonical JSON plus derived Markdown | embedded security design, review evidence, changed-file context, or persistence capability is missing |
