...
| Source-row prerequisite | When corporate source-row validation applies, require a readable non-blocking security review verdict that states exact source-row coverage, compact mapping validity, safe-evidence status, `N/A` justification status, warnings, exceptions, and blocker absence. Verification cites the summary and warnings only; it does not own or reproduce the full source-row matrix. |
...
5a. When corporate source-row validation applies, consume the security review source-row summary: expected row count, exact-once coverage statement, compact mapping status, safe-evidence status, `N/A` justification status, warnings, exceptions, blocker list, and source-row next route. Block unresolved source-row blockers before archive readiness.
...
12. Build the behavioral and security compliance matrices from actual test results and evidence when specs/scenarios/security controls exist; cite review and security-review summary evidence separately without duplicating either matrix or the full source-row matrix.
