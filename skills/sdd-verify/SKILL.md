...
7. Validate mandatory embedded secure-design evidence: every new change requires `design.md#secure-development-design`, while no-impact appears as narrative changed-surface rationale in design and source-row validation coverage, `N/A`, and `not-applicable` decisions belong to `review-security-report.md`.
