...
- Test runner: `package.json` scripts/deps, `app.json`, `app.config.js`, `app.config.ts`, `metro.config.js`, `react-native.config.js`, `android/`, `ios/`, `pyproject.toml`, `pytest.ini`, `go.mod`, `pom.xml`, `build.gradle`, `build.gradle.kts`, `settings.gradle`, `settings.gradle.kts`, `*.kt`, `*.kts`, `Cargo.toml`, `Makefile`.
- Test layers: unit runner; integration libraries (`testing-library`, `httpx`, `httptest`, `WebApplicationFactory`, Android/JUnit); E2E tools (`playwright`, `cypress`, `selenium`, `chromedp`, Detox, Maestro, Appium).
- Coverage: `vitest --coverage`, `jest --coverage`, `c8`, `pytest-cov`, `go test -cover`, `coverlet`, Gradle `test`/`jacocoTestReport` tasks.
...
`sdd-init/{project}` and `sdd/{project}/testing-capabilities` are both required in Engram-capable modes. Downstream planning phases read project context from `sdd-init/{project}`, while apply and verify phases read testing mode from `sdd/{project}/testing-capabilities`.
